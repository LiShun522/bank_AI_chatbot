...
# 讀取環境變數
qdrant_url = os.getenv("QDRANT_URL")          # Qdrant 伺服器 URL
qdrant_api_key = os.getenv("QDRANT_API_KEY")    # Qdrant API 金鑰
openai_api_key = os.getenv("OPENAI_API_KEY")      # OpenAI API 金鑰

# 初始化嵌入模型（此處使用 OpenAI 的模型作為示範）
...
# 切割文本：根據題號格式拆分，每組問答以兩位數字加句點開頭
parts = re.split(r'(?=\d{2}\.)', text.strip())

# 將每個切割後的部分包裝成 Document 物件
documents = [Document(page_content=part.strip()) for part in parts if part.strip()]
...
# 利用 Qdrant.from_documents 自動生成向量並上傳
from langchain.vectorstores import Qdrant

db = Qdrant.from_documents(
    documents,       # 這裡傳入 documents（也可以替換成您已切割好的 all_splits 列表）
    embeddings,
    url=qdrant_url,
    api_key=qdrant_api_key,
    collection_name="250206-PDF-3-small-final",  # 指定您的 collection 名稱
    batch_size=8
)

logging.info("成功上傳文件到 Qdrant。")
